function pav=myfundpavchrw(w0,A)
...
pav=sum(real(besselj(kxv,A/w1*xifun1(w0,A,r)).*besselj(-kxv,r*A/w2*xifun2(w0,A,r)).*xppz.*exp(-1i*kxv*dphi21))+2*real(exp(-1i*kxv*dphi21).*besselj(1-kxv,A/w1*xifun1(w0,A,r)).*besselj(kxv,r*A/w2*xifun2(w0,A,r)).*xppp));
end